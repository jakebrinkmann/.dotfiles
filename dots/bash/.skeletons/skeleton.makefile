...
REGISTRY := $(or $(DOCKER_USER), USERNAME)/
IMAGE := dotfiles
...
# For RPMs
NAME    =  $(shell cat *.spec | grep "Name:" | awk -F: '{print $$2}' | tr -d ' ')
VERSION =  $(shell cat *.spec | grep "Version:" | awk -F: '{print $$2}' | tr -d ' ')
PREVVER = $(shell echo $(VERSION) | awk -F. '{print $$1 "." $$2 "." $$3 - 1}')

.PHONY: image
image: $(TAG) ## Build the Dockerfile as an image.
